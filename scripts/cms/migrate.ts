...
for (const item of localContent.gallery) documents.push({ _id: `gallery-${item.id}`, _type: "galleryPhoto", permissionReminder: "Confirm this photograph is appropriate for public use. Take particular care with children and vulnerable people; do not store private consent records here.", stableId: { _type: "slug", current: item.id }, title: item.title, caption: item.caption, image: await imageReference(item.image), category: item.category, dateLabel: item.dateLabel, order: item.order });
